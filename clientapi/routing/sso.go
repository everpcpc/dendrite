...
func SSOCallback(
	...
	if localpart == "" {
		...
		if localpart == "" {
			...
			if err = userAPI.QueryNumericLocalpart(ctx, nreq, nres); err != nil {
			...
			localpart = strconv.FormatInt(nres.ID, 10)
		}
...
type userAPIForSSO interface {
	userapi.LoginTokenInternalAPI

	PerformAccountCreation(ctx context.Context, req *userapi.PerformAccountCreationRequest, res *userapi.PerformAccountCreationResponse) error
	PerformSaveSSOAssociation(ctx context.Context, req *userapi.PerformSaveSSOAssociationRequest, res *struct{}) error
	QueryLocalpartForSSO(ctx context.Context, req *userapi.QueryLocalpartForSSORequest, res *userapi.QueryLocalpartForSSOResponse) error
	QueryNumericLocalpart(ctx context.Context, req *userapi.QueryNumericLocalpartRequest, res *userapi.QueryNumericLocalpartResponse) error
}
...
func verifySSOUserIdentifier(ctx context.Context, userAPI userAPIForSSO, id *sso.UserIdentifier) (localpart string, _ error) {
	req := &userapi.QueryLocalpartForSSORequest{
		Namespace: id.Namespace,
		Issuer:    id.Issuer,
		Subject:   id.Subject,
	}
	var res userapi.QueryLocalpartForSSOResponse
...
func registerSSOAccount(ctx context.Context, userAPI userAPIForSSO, ssoID *sso.UserIdentifier, localpart string) (bool, util.JSONResponse) {
	...
	err = userAPI.PerformSaveSSOAssociation(ctx, &userapi.PerformSaveSSOAssociationRequest{
...
// user.
func createLoginToken(ctx context.Context, userAPI userAPIForSSO, userID string) (*userapi.LoginTokenMetadata, error) {
	req := userapi.PerformLoginTokenCreationRequest{Data: userapi.LoginTokenData{UserID: userID}}
	var resp userapi.PerformLoginTokenCreationResponse
